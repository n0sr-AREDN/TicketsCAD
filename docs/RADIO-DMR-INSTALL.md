...
The **bridge host** and the **TicketsCAD web host** can be the same VM (your deployment's setup) or separate (training has the bridge on `dvswitch-host`). Co-locating is simpler; separating is better if you'll run multiple TicketsCAD instances against one bridge.
...
On the host you're putting the bridge on (your deployment = same VM as the web; training = separate `dvswitch-host` VM):
