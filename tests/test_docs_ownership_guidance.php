 ...
 *   - BACKUP_DIR       = NEWUI_ROOT . '/backups'   (inc/backup.php), gitignored,
 *     created lazily by backup_run_now(); written by the CLI tool (as the
 *     operator) AND by api/backup.php + the documented cron line (as the web
 *     user) — hence shared ownership, not www-data ownership.
 *
...
test('BACKUP_DIR is inside the install directory',
    defined('BACKUP_DIR') && strpos($n(BACKUP_DIR), $n($root) . '/backups') === 0);
...
test('SWITCH-FROM-ZIP-TO-GIT.md shares backups/ instead of giving it away',
...
test('SWITCH-FROM-ZIP-TO-GIT.md notes backups/ is absent on a fresh clone',
    stripos($switch, 'mkdir -p backups') !== false);
...
test('check-health.php still suggests chown for the writable dirs',
    strpos($cli, 'chown -R $webUser:$webUser " . $d[\'abs\']') !== false);
